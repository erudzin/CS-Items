...
bool stupid_helper_number_2(Node *t1, Node *t2){
	...
	return stupid_helper_number_2(t1->left, t2->left) && stupid_helper_number_2(t1->right, t2->right);
}

bool contains_tree_helper(Node *t1, Node *t2){
	if(t1 == nullptr){
		return false;
	}
	if(t1 == t2){
		if(stupid_helper_number_2(t1, t2) == true){
			return true;
		}
	}
	return contains_tree_helper(t1->left, t2->left) || contains_tree_helper(t1->right, t2->right);
}
...
bool contains_tree(Node *t1, Node *t2){
	...
	return contains_tree_helper(t1, t2);
}
